...
class CalculationCapacityTest {
    ...
    void execute() throws Exception {
        CalculationCapacity calculationCapacity = new CalculationCapacity(airplanes);
        ByteOutputStream outputStream = new ByteOutputStream();
        PrintStream printStream = new PrintStream(outputStream);
        System.setOut(printStream);
        calculationCapacity.execute("".split(""));
        String sum = outputStream.toString().split(" ")[3];
        Assert.assertEquals(sum, "-250\r\n");
    }
}